...
| 模式 | 使用场景 | 示例 |
|---------|----------|---------|
| `Promise.all()` | 并行执行，快速失败 | `await Promise.all([p1, p2])` |
| `Promise.allSettled()` | 并行执行，获取所有结果 | `await Promise.allSettled([p1, p2])` |
| `Promise.race()` | 首个完成 | `await Promise.race([p1, p2])` |
| `Promise.any()` | 首个成功 | `await Promise.any([p1, p2])` |
| `async function*` | 异步迭代 | `for await (const x of gen())` |
| `AbortController` | 取消操作 | `fetch(url, { signal })` |
| `queueMicrotask()` | 优先微任务 | `queueMicrotask(fn)` |
