...
1. **资源管理**：始终设置请求和限制
2. **健康检查**：包含 liveness 和 readiness 探针
3. **安全**：使用非 root 用户，尽可能使用只读文件系统
4. **标签**：一致的标记以便于组织和选择
5. **更新策略**：选择适当的策略（RollingUpdate、Recreate）
6. **Service Accounts**：不要使用 default，创建特定的 SAs
7. **镜像标签**：生产环境使用特定版本，不要使用 `latest`
8. **清理**：为 Jobs 设置 TTL 以自动清理完成的 pods