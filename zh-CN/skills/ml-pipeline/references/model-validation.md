...
模型验证确保模型在生产部署前符合质量标准。它包含离线评估、在线测试和持续监控，以捕获性能下降、数据漂移和模型故障。
...
- `training-pipelines.md` - 验证前的模型训练
- `experiment-tracking.md` - 验证结果记录
- `pipeline-orchestration.md` - 自动化验证工作流
- `feature-engineering.md` - 特征验证
