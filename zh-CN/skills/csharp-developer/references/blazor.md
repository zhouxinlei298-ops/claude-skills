...
| `[Parameter]` | 组件输入 | 需要时使用 `[EditorRequired]` |
| `EventCallback` | 组件事件 | 类型安全的回调 |
| `RenderFragment` | 子内容 | 用于灵活布局 |
| `CascadingValue` | 共享状态 | 自动传递给后代 |
| `AuthorizeView` | 条件认证 UI | 或 `@attribute [Authorize]` |
