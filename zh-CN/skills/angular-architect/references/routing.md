...
| 特性 | 使用方式 |
|---------|-------|
| Routes | app.routes.ts 中的 Routes 数组 |
| 懒加载 | `loadComponent()`, `loadChildren()` |
| 守卫 | `CanActivateFn`, `CanDeactivateFn` |
| 解析器 | `ResolveFn<T>` |
| 参数 | `route.paramMap`, `input<T>()` |
| 查询参数 | `route.queryParamMap` |
| 导航 | `router.navigate()`, `routerLink` |
| 事件 | `router.events` |
| 出口 | `<router-outlet name="...">` |
| 预加载 | `withPreloading()` |