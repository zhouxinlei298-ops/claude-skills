...
| **精确率** | TP / (TP + FP) | 假正例成本高 |
| **召回率** | TP / (TP + FN) | 假负例成本高 |
| **F1 分数** | 2 * (P * R) / (P + R) | 不平衡类别 |
| **科恩 kappa 系数** | (准确率 - 预期) / (1 - 预期) | 评分者间一致性 |
...
| 指标 | 测量内容 | 局限性 |
|------|----------|--------|
| **BLEU** | 与参考的 N 元语法重叠 | 无法捕捉语义 |
| **ROUGE** | 参考的 N 元语法召回率 | 更适合摘要任务 |
| **BERTScore** | 通过嵌入的语义相似性 | 计算成本高 |
| **困惑度** | 模型置信度 | 不衡量正确性 |
...
{
  "accuracy": <1-5>,
  "accuracy_reasoning": "<简要解释>",
  "relevance": <1-5>,
  "relevance_reasoning": "<简要解释>",
  "clarity": <1-5>,
  "clarity_reasoning": "<简要解释>",
  "completeness": <1-5>,
  "completeness_reasoning": "<简要解释>",
  "overall_score": <1-5>,
  "summary": "<一句话总结>"
}
```
