...
| 模式 | 用途 |
|---------|-------|
| `RequireAuthorization()` | 端点需要身份验证 |
| `RequireAuthorization("Policy")` | 端点需要特定策略 |
| `AllowAnonymous()` | 允许未经身份验证的访问 |
| `RequireRole("Admin")` | 需要特定角色 |
| JWT Bearer | 基于令牌的身份验证 |
| `ICurrentUserService` | 访问当前用户信息 |
| `IPasswordHasher` | 哈希和验证密码 |
| `IJwtService` | 生成和验证令牌 |
